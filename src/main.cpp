...
#if defined(NDEBUG)
# error "Rsvcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xec4b9ba4b3d285996a79b6657757debabbf479e320e60c4e41a8cd36c6989902");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rsvcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rsvcoin Signed Message:\n";
...
{
    // Rsvcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Rsvcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Rsvcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Rsvcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rsvcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rsvcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Rsvcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x127aee2faa11ecded3c57e5353e8103199dd24d74f48ae2dfb6a9b29e6b596bd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "MD 24/Apr/2014 nuru is 50 year old ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398502317;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 603051;

        if (fTestNet)
        {
            block.nTime    = 1398503013;
            block.nNonce   = 4349078;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x551b5c0a090f35829c1dece1cfeb73eda4728383bf6e90d0e66a6906d11b4db6"));
    block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Rsvcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RsvcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RsvcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RsvcoinMiner(CWallet *pwallet)
{
    printf("RsvcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rsvcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RsvcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RsvcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RsvcoinMiner, pwallet));
}
